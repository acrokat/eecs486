...
class TwitterClient(object):
	...
	def is_fan(self,username, team):
		...
		for i, page in enumerate(tweepy.Cursor(self.api.friends_ids, id=self.api.get_user(username).id, count=200).pages()):
			for userid in page:
				if str(NFL_Dictionary[team]) == str(userid):
...
def main():
	...
	for handle in NFL_HANDLES:
		NFL_Dictionary[handle[1:]] = NFL_TEAMS[i]
		i+=1
	
	statFile = open("nflStats.txt", 'w')
	

	for handle in NFL_HANDLES:
		fileName = handle[1:]+"Tweets.txt"
		tweetFile = open(fileName, 'w')
		statFile.write( "Tweets about "+str(handle))
		statFile.write('\n')
		# calling function to get tweets
		tweets = api.get_tweets(query = handle, count = 100)
		# picking positive tweets from tweets
		ptweets = [tweet for tweet in tweets if tweet['sentiment'] == 'positive']
		# percentage of positive tweets
		statFile.write(("Positive tweets percentage: {} %".format(100*len(ptweets)/len(tweets))))
		statFile.write('\n')
		# picking negative tweets from tweets
		ntweets = [tweet for tweet in tweets if tweet['sentiment'] == 'negative']
		# percentage of negative tweets
		statFile.write(("Negative tweets percentage: {} %".format(100*len(ntweets)/len(tweets))))
		statFile.write('\n')
		# percentage of neutral tweets
		statFile.write(("Neutral tweets percentage: {} % ".format(100*(len(tweets) - len(ntweets) - len(ptweets))/len(tweets))))
		statFile.write('\n')
	 	errors = 0
	 	correct = 0
		...
		for tweet in tweets:
			try:
				tweetFile.write(str(tweet['text']) + " " + str(tweet['sentiment']))
				tweetFile.write('\n')
				correct += 1
			except:
				errors+= 1
				continue
		print "done with",handle,str(correct),"-",str(errors),str(correct+errors)
